...
두 기본 폰트 모두에 없는 글리프는
[D2Coding](https://github.com/naver/d2codingfont)에서 채웁니다. 한자, 가나,
`①` 같은 원문자, `㈀` 같은 한글 원문자, 확장 화살표가 여기에 해당합니다.
채워지는 글리프는 D2Coding 자체의 반각/전각 관례를 따르므로 정확히 라틴
...
- `--fallback-dir`: 두 기본 폰트에 없는 글리프를 채우는 데 사용하는
  `D2Coding-*.ttf`가 들어 있는 디렉터리
...
D2Coding은 Regular와 Bold만 제공하므로, fallback 글리프는 Thin부터 Medium까지
`D2Coding-Regular`를, SemiBold부터 ExtraBold까지 `D2Coding-Bold`를 upright와
italic 변형 모두에서 사용합니다.
